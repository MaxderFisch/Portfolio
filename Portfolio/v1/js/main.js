/* =========================================================
   Max Aulinger — Portfolio (V1 · Monochrom)
   Haupt-JS: Navigation, Justified Gallery, Lightbox,
   Scroll-Reveal, Fullscreen-Toggle, Cursor-Punkt
   ========================================================= */
...
/* ---------------------------------------------------------
   Navigation: Scroll-Effekt + mobiles Burger-Menü
   --------------------------------------------------------- */
function initNav() {
...
/* ---------------------------------------------------------
   Galerie: Justified Layout (Lightroom-Stil) aus PHOTOS-Array
   --------------------------------------------------------- */
let currentPhotos = [];
...
function initGallery() {
  ...
  // Nach vollständigem Laden (Layout/Fonts) noch einmal neu berechnen
...
function renderGallery(grid, photos) {
  ...
  // Neu gerenderte Elemente in die Scroll-Reveal-Beobachtung aufnehmen
...
/**
 * Ordnet die Fotos in Reihen mit einheitlicher Höhe an (wie ein
 * Lightroom-/Flickr-Export): Querformate nehmen mehr Platz ein,
 * Hochformate weniger — kein Bild wird beschnitten.
 */
function layoutJustifiedGallery(grid) {
  ...
  rows.forEach(({ row, rowNaturalWidth, gapsWidth, isLast }) => {
    ...
    // Die letzte, evtl. unvollständige Reihe nicht aufblähen (bleibt ggf. linksbündig kürzer)
...
/* ---------------------------------------------------------
   Lightbox: großformatige Ansicht mit Vor/Zurück
   --------------------------------------------------------- */
let lightboxIndex = 0;
...
/* ---------------------------------------------------------
   Fullscreen-Toggle: kleiner Button unten rechts
   --------------------------------------------------------- */
...
/* ---------------------------------------------------------
   Cursor-Punkt: folgt der Maus, wächst über klickbaren
   Elementen zu einem Ring (nur bei echten Maus-Geräten)
   --------------------------------------------------------- */
function initCursorDot() {
  ...
  const hoverTargets = "a, button, .photo-item, .filter-btn, input, textarea";
